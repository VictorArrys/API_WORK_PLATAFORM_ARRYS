...
path.patch('/v1/perfilAspirantes/:idPerfilAspirante/video', multerUpload.single('video'), (req, res) => {
    ...
    try {
        GestionUsuarios.patchVideo(req.file.buffer, idPerfilAspirante, function(codigoRespuesta, cuerpoRespuesta){
            res.status(codigoRespuesta)
            res.json(cuerpoRespuesta)
        })
    } catch (error) {
        consoleError(error, 'Funcion: registro nuevo video. Paso: Excepcion cachada')
...
path.get('/v1/perfilAspirantes/:idPerfilAspirante/video', (req, res) => {
    ...
    try {
        if (respuesta.statusCode == 200){
            GestionUsuarios.getVideoAspirante(idPerfilAspirante, function(codigoRespuesta, cuerpoRespuesta){
...
path.get('/v1/perfilAspirantes/:idPerfilUsuarioAspirante', (req, res) => {
    ...
    try {
        if (respuesta.statusCode == 200){
            GestionUsuarios.getAspirante(idPerfilUsuarioAspirante, function(codigoRespuesta, cuerpoRespuesta) {
                res.status(codigoRespuesta)
                res.json(cuerpoRespuesta)
            })
        }else if (respuesta.statusCode == 401){
...
path.post('/v1/perfilAspirantes', (req, res) => {
    ...
    try {
        ...
        GestionUsuarios.postAspirante(aspirante, function(codigoRespuesta, cuerpoRespuesta) {
            res.status(codigoRespuesta)
            res.json(cuerpoRespuesta)
        })
        ...
        consoleError(error, 'Funcion: Registrar Aspirante. Paso: Excepcion cachada')
...
path.put('/v1/perfilAspirantes/:idPerfilAspirante', (req, res) => {
    ...
    try {
        if (respuesta.statusCode == 200){
            ...
            GestionUsuarios.putAspirante(edicionAspirante, function(codigoRespuesta, cuerpoRespuesta){
                res.status(codigoRespuesta)
                res.json(cuerpoRespuesta)
            })
        ...
        consoleError(error, 'Funcion: modificar un aspirante. Paso: Excepcion cachada')
