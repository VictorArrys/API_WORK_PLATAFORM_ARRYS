...
exports.MensajeDAO = class MensajeDAO {
  static postMensajeDemandante(
    ...
    var queryConversacion =
      "INSERT INTO mensaje (id_conversación_mensaje, id_usuario_remitente, mensaje, fechaRegistro) " +
      "VALUES (?,(SELECT id_perfil_usuario_demandante FROM perfil_demandante WHERE id_perfil_demandante = ?),?,NOW());";

    mysqlConnection.query(
      queryConversacion,
      [idConversacion, idDemandante, mensaje],
      (error, resultadoMensaje) => {
        if (error) {
  ...
  static postMensajeAspirante(idAspirante, idConversacion, mensaje, callback) {
    ...
    mysqlConnection.query(
      queryConversacion,
      [idConversacion, idAspirante, mensaje],
      (error, resultadoMensaje) => {
        if (error) {
          callback(500, mensajes.errorInterno);
        } else {
  ...
  static postMensajeEmpleador(idEmpleador, idConversacion, mensaje, callback) {
    ...
    mysqlConnection.query(
      queryConversacion,
      [idConversacion, idEmpleador, mensaje],
      (error, resultadoMensaje) => {
        if (error) {
          callback(500, mensajes.errorInterno);
        } else {
