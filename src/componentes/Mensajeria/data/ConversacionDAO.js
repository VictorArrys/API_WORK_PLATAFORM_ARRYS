...
const { ConversacionDemandante, ConversacionAspirante, ConversacionEmpleador } = require('../datatype/Conversacion');
const { ConversacionesAspirante, ConversacionesDemandante, ConversacionesEmpleador } = require('../datatype/Conversaciones');
...
exports.ConversacionDAO = class ConversacionDAO {
    ...
    static getConversacionAspirante(idAspirante, idConversacion, callback){
        ...
        mysqlConnection.query(queryConversaciones, [idAspirante, idConversacion], (error, resultadoConversacion) => {
            if(error){ 
                callback(500,mensajes.errorInterno);
            } else {
                if (resultadoConversacion.length == 1){
                    MensajeDAO.getMensajes(idConversacion, (codigoRespuesta, cuerpoRespuesta) => {
                        if (codigoRespuesta == 200) {
                            ...
                            conversacionAspirante.idConversacion =  resultadoConversacion[0]['id_conversacion'];
                            conversacionAspirante.tituloEmpleo = resultadoConversacion[0]['nombre_empleo'];
    ...
    static getConversacionesEmpleador(idEmpleador, callback) {
        ...
        mysqlConnection.query(queryConversaciones,[idEmpleador], (error, resultado) => {
            if(error){ 
                callback(500, mensajes.errorInterno);
            } else {
                const conversaciones = [];
    ...
    static getConversacionEmpleador(idEmpleador, idConversacion, callback){
        ...
        mysqlConnection.query(queryConversacion, [idEmpleador, idConversacion], (error, resultado) => {
            if(error){ 
                callback(500, mensajes.errorInterno)
            } else if(resultado.length == 0){
                callback(404, mensajes.peticionNoEncontrada)
