...
exports.AspiranteDAO = class AspiranteDAO {
      ...
      if (error) {
        ...
        if (resultadoConsulta.length == 0) {
          ...
          resultadoConsulta.forEach((elemento) => {
            ...
            OficioDAO.getOficios(
              aspirante.idPerfilAspirante,
              (errorOficios, resultadoOficios) => {
                if (errorOficios) {
  ...
  static getAspirante(idUsuario, callback) {
    var query =
      "SELECT * FROM perfil_aspirante WHERE id_perfil_usuario_aspirante = ?;";
  ...
  static getVideoAspirante(idAspirante, callback) {
    ...
    mysqlConnection.query(query, [idAspirante], (error, video) => {
      if (error) {
        ...
        var arrayVideo = null;
        arrayVideo = Uint8Array.from(
          Buffer.from(video[0]["video"].buffer, "base64")
        );
  ...
  static patchVideoAspirante(video, idAspirante, callback) {
    var query =
      "UPDATE perfil_aspirante SET video = ? WHERE id_perfil_aspirante = ?;";

    mysqlConnection.query(
      query,
      [video, idAspirante],
      (error, resultadoVideo) => {
        if (error) {
          callback(500, mensajes.errorInterno);
  ...
  static postAspirante(aspiranteNuevo, callback) {
    var queryTwo =
      "INSERT INTO perfil_aspirante ( id_perfil_usuario_aspirante, nombre, direccion, fecha_nacimiento, telefono) VALUES (?, ?, ?, ?, ?); ";
    var queryThree =
      "INSERT INTO categoria_aspirante (id_aspirante_ca, id_categoria_ca, experiencia) VALUES ? ;";

    this.#comprobarRegistro(
      aspiranteNuevo.nombreUsuario,
      aspiranteNuevo.correoElectronico,
            ...
            function (codigoRespuesta, cuerpoRespuesta) {
              if (codigoRespuesta == 500) {
                ...
                mysqlConnection.query(
                  ...
                  [
                    idUsuario,
                    aspiranteNuevo.nombre,
                    aspiranteNuevo.direccion,
                    aspiranteNuevo.fechaNacimiento,
                    aspiranteNuevo.telefono,
                  ],
                  (error, registroPerfilAspirante) => {
                    if (error) {
                      console.log(error)
                      callback(500, mensajes.errorInterno);
                      ...
                      do {
                        valores[cont] = [
                          idPerfil,
                          aspiranteNuevo.oficios[cont].idCategoria,
                          aspiranteNuevo.oficios[cont].experiencia,
                        ];
                        cont++;
  ...
  static putAspirante(aspirante, callback) {
    var queryTwo =
      "UPDATE perfil_aspirante SET nombre = ?, direccion = ?, fecha_nacimiento = ?, telefono = ? WHERE id_perfil_aspirante = ?;";
    var queryThree =
      "INSERT INTO categoria_aspirante (id_aspirante_ca, id_categoria_ca, experiencia) VALUES ? ;";
            ...
            function (codigoRespuesta, cuerpoRespuesta) {
              if (codigoRespuesta == 500) {
                ...
                mysqlConnection.query(
                  ...
                  (error, actualizacionPerfilAspirante) => {
                    if (error) {
                      callback(500, mensajes.errorInterno);
                      ...
                      do {
                        valores[cont] = [
                          aspirante.idPerfilAspirante,
                          aspirante.oficios[cont].idCategoria,
                          aspirante.oficios[cont].experiencia,
                        ];
                        cont++;
