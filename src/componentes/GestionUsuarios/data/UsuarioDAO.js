...
exports.UsuarioDAO = class UsuarioDAO {
      ...
      if (error) {
        ...
        if (resultadoUsuario[0].fotografia == null) {
          ...
          arrayFotografia = Uint8ClampedArray.from(
            Buffer.from(resultadoUsuario[0].fotografia.buffer, "base64")
          );
        }
  ...
  static postUsuario(usuario, callback) {
    var query =
      "INSERT INTO perfil_usuario (nombre_usuario, estatus, clave, correo_electronico, tipo_usuario) VALUES (?, ?, ?, ?, ?);";
    ...
    mysqlConnection.query(
      ...
      [
        ...
        usuarioPerfil.estatus,
        usuarioPerfil.clave,
        usuarioPerfil.correoElectronico,
        usuarioPerfil.tipoUsuario,
      ],
      (error, resultadoRegistroUsuario) => {
        if (error) {
          console.log(error)
          callback(500, mensajes.errorInterno);
  ...
  static putUsuario(usuario, callback) {
    ...
    mysqlConnection.query(
      ...
      [
        ...
        usuarioEdicion.clave,
        usuarioEdicion.correoElectronico,
        usuarioEdicion.idPerfilUsuario,
      ],
      (error, edicionUsuario) => {
        if (error) {
          console.log(error)
          callback(500, mensajes.errorInterno);
  ...
  static patchFotografiaUsuario(idUsuario, fotografia, callback) {
    var query =
      "UPDATE perfil_usuario SET fotografia = ? WHERE id_perfil_usuario = ?;";

    mysqlConnection.query(
      query,
      [idUsuario, fotografia],
      (error, resultadoFotografia) => {
        if (error) {
          console.log(error)
          callback(500, mensajes.errorInterno);
