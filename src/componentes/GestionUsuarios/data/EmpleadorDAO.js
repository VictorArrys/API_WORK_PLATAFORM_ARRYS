...
exports.EmpleadorDAO = class EmpleadorDAO {
    ...
    static postEmpleador(empleadorNuevo, callback) {
        ...
        this.#comprobarRegistro(empleadorNuevo.nombreUsuario, empleadorNuevo.correoElectronico, function(codigoRespuesta, cuerpoRespuestaRegistro){
            if (codigoRespuesta == 500){
                callback(500, mensajes.errorInterno)
            }else if (cuerpoRespuestaRegistro >= 1){
                callback(422, mensajes.instruccionNoProcesada)
            }else{
                UsuarioDAO.postUsuario(empleadorNuevo, function(codigoRespuesta, cuerpoRespuesta){
                    if (codigoRespuesta == 500){
                        callback(500, mensajes.errorInterno)
                        ...
                        mysqlConnection.query(queryTwo, [idUsuario, empleadorNuevo.nombreOrganizacion, empleadorNuevo.nombre, empleadorNuevo.direccion, empleadorNuevo.fechaNacimiento, empleadorNuevo.telefono, empleadorNuevo.amonestaciones], (error, registroPerfilEmpleador) =>{
                            if (error){
                                console.log(error)
                                callback(500, mensajes.errorInterno)
    ...
    static putEmpleador(empleador, callback) {
        var query = 'UPDATE perfil_empleador SET nombre_organizacion = ?, nombre = ?, direccion = ?, fecha_nacimiento = ?, telefono = ? WHERE id_perfil_empleador = ? ;'

        UsuarioDAO.putUsuario(empleador, function(codigoRespuesta, cuerpoRespuesta){
            if (codigoRespuesta == 500){
                callback(500, mensajes.errorInterno)
            }else if (codigoRespuesta == 404){
                callback(404, mensajes.peticionNoEncontrada)
            }else{
                mysqlConnection.query(query, [empleador.nombreOrganizacion, empleador.nombre, empleador.direccion, empleador.fechaNacimiento, empleador.telefono, empleador.idPerfilEmpleador], (error, actualizacionPerfilEmpleador) => {
                    if (error){
                        console.log(error)
                        callback(500, mensajes.errorInterno)
