...
exports.AdministradorDAO = class AdministradorDAO {
    ...
    static putAdministrador(administrador, callback) { // falta mandar el error de la funcion
        var queryTwo = 'UPDATE perfil_administrador SET nombre = ?, telefono = ? WHERE id_perfil_administrador = ?;'
        this.#actualizarUsuarioAdministrador(administrador, function(codigoRespuesta, cuerpoRespuesta){
            if (codigoRespuesta == 500){
                callback(500, mensajes.errorInterno)
            }else if (codigoRespuesta == 404){
                callback(404, mensajes.peticionNoEncontrada)
            }else{
                mysqlConnection.query(queryTwo, [administrador.nombre, administrador.telefono, administrador.idPerfilAdministrador], (error, actualizacionAdministrador) =>{
                    if (error){
                        callback(500, mensajes.errorInterno)
    ...
    static #actualizarUsuarioAdministrador(administrador, callback){
        ...
        mysqlConnection.query(queryOne, [usuario.nombreUsuario, usuario.clave, usuario.correoElectronico, usuario.idPerfilUsuario], (error, actualizacionUsuario) =>{
            if (error){
                callback(500, mensajes.errorInterno)
